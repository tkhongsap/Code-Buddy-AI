...
import ScoreCodeHeader from "./ScoreCodeHeader";
...
export default function ScoreCodeInterface() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      sender: 'ai',
      content: 'Hi there! I\'m your Code Quality Analyst. Paste your code, and I\'ll evaluate it on a scale of 1-10 and provide detailed feedback on strengths and areas for improvement.',
  ...
  const sendMessageMutation = useMutation({
    ...
    onError: (error) => {
      ...
      toast({
        title: "Error",
        description: "Failed to communicate with scoring service. Please try again.",
        variant: "destructive",
      });
      ...
      setMessages(prev => [
        ...prev,
        {
          id: Date.now(), // Use timestamp for uniqueness
          sender: 'ai',
          content: "I'm sorry, I'm having trouble connecting to the code scoring service. Please try again in a moment.",
          timestamp: new Date().toLocaleTimeString()
        }
      ]);
    }
  ...
  const saveResponse = (messageId: number) => {
    toast({
      ...
      description: "Saving code scoring responses is coming soon!",
      variant: "default",
    });
  };
  ...
  const handleStreamedResponse = async (userQuestion: string, conversationHistory: Message[]) => {
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to communicate with code scoring service. Please try again.",
        variant: "destructive",
      });
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      <Header />
      <ScoreCodeHeader toggleStreaming={toggleStreaming} useStreaming={useStreaming} />
